...
\begin{figure}[h]
    ...
    y ajustada para las distribuciones Normal ($\mu=5,\,\sigma=2$), 
    Exponencial ($\lambda=0.5$) y t de Student ($\nu=5$). 
    En la fila inferior se muestran los intervalos de confianza 
    del 95\% para la varianza (línea azul), la varianza muestral 
    $S_n^2$ (cuadrado naranja) y la varianza poblacional real 
    $\sigma^2$ (línea vertical roja punteada).}
