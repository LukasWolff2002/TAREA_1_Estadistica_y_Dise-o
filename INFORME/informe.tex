...
\begin{document}
    ...
    \textbf{\Large Estudio de Distribuciones de Probabilidad: Análisis de Datos y Aplicaciones en Estadística}
...
La distribución t de Student surge al estimar la media de una población normal 
...
Respecto al Teorema de Cochran, la t de Student \textbf{no satisface} sus 
...
El Teorema de Cochran establece una propiedad fundamental de la distribución 
...
La condición crítica del teorema es la \textbf{normalidad de los datos}. Si los 
...
\begin{figure}[h]
    ...
    no lo cubre. Cobertura en 1000 repeticiones: $95.5\%$ (Normal), 
    $95.6\%$ (Exponencial) y $86.2\%$ (t de Student).}
...
\textbf{se cumple exactamente}: bajo normalidad, $(n-1)S_n^2/\sigma^2 
...
de Cochran \textbf{no aplica de forma exacta}, pero con $n=100$ el Teorema 
...
el triple de las veces esperadas. El Teorema de Cochran \textbf{no se 
cumple} con $n=100$. La causa son las colas pesadas de la t de Student, 
cuya curtosis $\kappa = 6/(\nu-4)$ diverge para $\nu \to 4^+$, impidiendo 
que $S_n^2$ converja a $\chi^2_{99}$ con rapidez suficiente. En 
...
superior queda a la izquierda de $\sigma^2$. El resultado \textbf{sí cambia 
...
de tamaño moderado. Esto subraya que la elección de la distribución no 
es un detalle menor: aplicar el estadístico $\chi^2$ fuera de su dominio 
de validez produce inferencias incorrectas cuya gravedad depende 
críticamente del comportamiento de las colas de la distribución subyacente.
